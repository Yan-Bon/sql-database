...
size_t RBTreeInfoIndex::computeHash(std::string& info) {
    std::hash<std::string> hasher;
    return hasher(info);
}

IndexTreeNode* RBTreeInfoIndex::searchNode(size_t hash) {
...
void RBTreeInfoIndex::rotateLeft(IndexTreeNode* x) {
...
void RBTreeInfoIndex::rotateRight(IndexTreeNode* x) {
...
void RBTreeInfoIndex::fixInsert(IndexTreeNode* k) {
...
IndexTreeNode* RBTreeInfoIndex::minimum(IndexTreeNode* node) {
...
void RBTreeInfoIndex::transplant(IndexTreeNode* u, IndexTreeNode* v) {
    if (u->parent == nullptr) {
        root = v;
...
void RBTreeInfoIndex::fixDelete(IndexTreeNode* x) {
...
void RBTreeInfoIndex::insert(StudentEntity& entity, guid id) {
    size_t hash = computeHash(entity.info);
...
StudentEntity* RBTreeInfoIndex::search(std::string& info) {
    size_t hash = computeHash(info);
    IndexTreeNode* node = searchNode(hash);
    return node ? storage->Read(node->id) : nullptr;
}

bool RBTreeInfoIndex::remove(std::string& info) {
    size_t hash = computeHash(info);
...
bool RBTreeInfoIndex::contains(std::string& info) {
    size_t hash = computeHash(info);
    IndexTreeNode* node = searchNode(hash);
